  ...
  return (
    ...
    <div className="flex flex-col min-h-screen">
      ...
      <div className="flex flex-1 mt-22">
        ...
        {/* Main content */}
        <main className="w-full sm:flex-1 relative pt-0 sm:pt-2">
          {/* cartHeader */}
{           <div className="w-full ml-2 h-14 sm:h-16 border border-r-0 border-t-0 border-l-0 border-gray-300 text-maintext  flex-row-center justify-start pl-2 pr-2 sm:pl-6 sm:pr-6 bg-bg sm:bg-bg-light absolute top-0 ">
              <p className="font-semibold sm:text-lg">  <FontAwesomeIcon icon={faShoppingCart} className="text-lg sm:text-xl text-accent"/> Cart Manager</p>
            </div>}
          ...
          <div className="w-full p-2 lg:p-8 pt-8 h-[74vh] overflow-y-scroll">
                {/* Cart Items */}
                {activeTab === "cart" && (
                  <div className="pt-6 sm:pt-11">
                    <h2 className="text-lg lg:text-lg mb-4 text-primary hidden sm:block">Available Cart Items</h2>
                    {cartItems.length === 0 ? (
                      ...
                      <div className="flex flex-col items-end justify-evenly">
                        {cartItems.map((item) => (
                          <div key={item.id} className="h-fit w-full flex justify-between items-center shadow-sm p-1 sm:p-4 rounded mb-2 flex-wrap ">
                              <div className="w-fit h-fit flex-row-center flex-wrap">
                                {/* item image */}
                                  <img
                                      src={item.image}
                                      alt={item.title}
                                      className="m-auto w-20 h-20 sm:w-36 sm:h-30 object-cover rounded-lg mr-2 sm:mr-4 bg-bg-dark text-sm border border-gray-300"
                                  />
                      
                                {/* item name and price */}
                                  <div className="w-fit h-30 flex-col-start justify-start">
                                         <h4 className="font-semibold pt-5 text-sm">{item.sizeKg}kg {item.title}</h4>
                                         <p className="text-text text-xs sm:text-sm mb-4 mt-0.5">Total item cost: <span className="text-primary text-sm font-semibold">${(item.price * (item.sizeKg) * item.qty).toFixed(2)}</span></p>
                          ...
                          {/* remove button */}
                            <button
                              className="text-accent bg-bg p-1 sm:p-2 rounded-md hover:bg-bg-dark cursor-pointer duration-300 text-sm hidden sm:flex"
                          ...
                          {/* button for mobiles */}
                            <button
                              className="text-accent rounded-md hover:brightness-75 cursor-pointer duration-300 text-sm sm:hidden"
                              onClick={() => removeFromCart(item.id, item.sizeKg)}
                            >
                          ...
                          <div className="w-full sm:w-80 h-30 shadow-sm rounded-lg mt-4 flex-col-center justify-evenly p-4 py-2 mb-8 ">
                              <h3 className=" font-semibold w-full flex items-center justify-between">Grand Total: <span className="text-primary font-bold">${total.toFixed(2)}</span></h3>
                ...
                {activeTab === "history" && (
                  <div className="mb-10 mt-10">
                    <h2 className="text-lg  sm:mb-4 text-primary hidden sm:block">Cart History</h2>
                    ...
                    ( [...cartHistory].reverse().map((cart, index) => (
                      <div key={cart.id}>
                          ...
                          <div>
                            {cart.items.map((item) => (
                              <div key={`${item.id}_${item.sizeKg}`} className="shadow-sm mb-3">
                                  <div className="w-fit h-fit flex-row-center flex-wrap p-1">
                                    {/* item image */}
                                      <img
                                          src={item.image}
                                          alt={item.title}
                                          className="m-auto text-xs sm:text-sm font-light w-20 h-20 sm:w-36 sm:h-30 object-cover rounded-lg mr-2 sm:mr-4 bg-bg-dark border border-gray-300"
                                      />

                                    {/* item name and price */}
                                    <div className="w-fit h-26 flex-col-start justify-center">
                                          <h4 className="font-semibold text-sm tracking-wide sm:text-base">{item.title}</h4>
                                          <h4 className="text-xs sm:text-sm">{item.sizeKg}kg x {item.qty} </h4>
                                          <p className="text-text text-xs mb-4 mt-0.5 sm:text-sm">Total cost: <span className="text-primary text-sm font-semibold">${(item.price * (item.sizeKg) * item.qty).toFixed(2)}</span></p>
                                    </div>
                                  </div>
                              </div>
                            ))}
                          ...
                          <div className="w-full border-b-2 border-text/20 pb-6 flex-row-center justify-between mb-6">
                              {/* total order cost */}
                              <h4 className="mt-2">
                                  Order Total: <span className="text-primary font-semibold">${cart.items.reduce((sum, item) => sum + item.price * item.sizeKg * item.qty,0)}</span>
                              </h4>

                              {/* clear cart history button */}
                              <button
                                    className="mt-4 bg-accent cursor-pointer text-white px-1.5 py-2 rounded text-xs sm:text-sm"
